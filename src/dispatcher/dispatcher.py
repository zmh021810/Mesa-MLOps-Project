...
def get_latest_model_key(bucket, prefix):
    ...
    print(f"✅ 找到最新模型: {latest_model['Key']} (更新时间: {latest_model['LastModified']})")
...
def handler(event, context):
    try:
        ...
        if action == 'train':
            ...
            return {
                "statusCode": 200,
                "body": json.dumps({"msg": "Training job started", "job_name": job_name})
            }
        ...
        # --- 动作 B: 触发自动预测 ---
            ...
            payload = {
                ...
                "data_key": body.get('data_key', 'test/test_data.csv') # 默认测试集路径
            }
